...
def main():
    instructions = [parse(instruction) for instruction in Path(
        "inputs/day18.txt").read_text().splitlines()]
    registers = {}
    ...
    p1_target = []
    p1 = Program(0, registers.copy(), [])
    p1.target = p1_target
    finished = False
    while not finished:
        finished = p1.tick(instructions)
    print(f"Part 1: {p1_target[-1]}")

    p1 = Program(0, registers.copy(), [])
    p2 = Program(1, registers.copy(), [])
    p1.target = p2.queue
    p2.target = p1.queue
    curr = p2
    prev_stalled = False
    while True:
        curr_stalled = curr.tick(instructions)
        if curr_stalled:
            if prev_stalled:
                break
            curr = p2 if curr == p1 else p1
            prev_stalled = True
        else:
            prev_stalled = False
    print(f"Part 2: {p2.sent_times}")
